...
vector<vector<Job*>> edd(vector<Job*> jobs,int machines) {
    // Ordenar los trabajos por due date (ddate)
    ...
    // Asignar los trabajos a las máquinas
    vector<vector<Job*>> machine_jobs(machines);
    for (size_t i = 0; i < jobs.size(); ++i) {
        machine_jobs[i % machines].push_back(jobs[i]);
    }
...
vector<vector<Job*>> mst(vector<Job*> jobs,int machines) {
    // Ordenar los trabajos por slack time (ddate - ptime), y por ptime si el slack time es igual
    sort(jobs.begin(), jobs.end(), [](Job* a, Job* b) {
        int slackA = a->dDate - a->ptime;
        int slackB = b->dDate - b->ptime;
        if (slackA == slackB) {
            return a->ptime < b->ptime;
        }
...
vector<vector<Job*>> spt(vector<Job*> jobs,int machines) {
    // Ordenar los trabajos por processing time (ptime), y por ddate si ptime es igual
    sort(jobs.begin(), jobs.end(), [](Job* a, Job* b) {
        if (a->ptime == b->ptime) {
            return a->dDate < b->dDate;
        }
